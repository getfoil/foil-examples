...
class SupportAgent {
  ...
  async processMessage(sessionId, userMessage, ctx, { imagePath } = {}) {
    ...
    // Build span input — use SDK content blocks for image tracing
    const spanInput = isImage
      ? content(textPart || 'Customer attached this image:', ContentBlock.file(filePath))
      : userMessage;

    // One LLM span per conversational turn — stays open through the entire tool loop
    // so tool spans nest underneath, and the final output is the actual text response.
    const span = await ctx.startSpan('llm', this.model, {
      input: spanInput,
      properties: { turn: convo.turnCount, sessionId },
      llmConfig: { temperature: 0.3, maxTokens: 1024 },
    });
    ...
    try {
      ...
      while (response.choices[0].finish_reason === 'tool_calls' || response.choices[0].message.tool_calls) {
        ...
        // Execute tools — creates child spans nested under the LLM span
      ...
      // End the turn span with the actual assistant text and accumulated tokens
      await span.end({
        output: finalMessage.content,
        tokens: totalTokens.total > 0 ? totalTokens : undefined,
      });
